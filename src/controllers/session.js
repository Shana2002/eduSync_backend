...
function checkSessionName(req, callback) {
  ...
  db.query(q, [moduleid, batch_id], (err, data) => {
    ...
    if (sessions === current_sessions) {
      return callback(
        new Error("This module has already completed its sessions")
      );
    }
  ...
  try {
    checkToken(req, res, "secretkeyAdmin", (err, userInfo) => {
      ...
      checkAvaialabelHalls(req, (err, availableHalls) => {
        ...
        if (
          !Array.isArray(availableHalls) ||
          !availableHalls.includes(hall_id)
        ) {
          return res
            .status(400)
            .json({ message: `Hall no ${hall_id} is already booked` });
        }
    ...
    checkToken(req, res, "secretkeyAdmin", (err, userInfo) => {
      ...
      checkAvaialabelHalls(req, (err, data) => {
        ...
        if (!data.includes(hall_id))
          return res
            .status(400)
            .json({ message: `hall no ${hall_id} already booked` });
...
export const getSessionName = (req, res) => {
  ...
  if (!module || !batch)
    return res
      .status(400)
      .json({ message: "Missing required parameters: name and age" });
  ...
  db.query(q, [batch, module], (err, data) => {
    ...
    if (!data || data.length === 0) {
      ...
      return res
        .status(200)
        .json({
          session: `Session ${data.length + 1}`,
          lecture: data[0].first_name + ` ` + data[0].last_name,
          module_assign: data[0].module_assign_id,
        });
    }
  });
};
...
export const getSessionsStudent = (req, res) => {
  checkToken(req, res, "secretkey", (err, userInfo) => {
    if (err) return res.status(500).json(err.message);
    const { date } = req.params;
    console.log(userInfo.id);
    const batchQuery =
      "SELECT * FROM student_enroll se WHERE se.student_id = ? LIMIT 1";
    db.query(batchQuery, [userInfo.id], (err, data) => {
      if (err) return res.status(500).json(err);
      ...
      const q = `SELECT * FROM shedule AS sh 
            ...
            LEFT JOIN module_assign AS ma ON ma.module_assign_id = s.module_asign_id
            LEFT JOIN module AS m ON m.module_id = ma.module_id
            WHERE sh.shedule_date= ? AND ma.batch_id= ?`;
      db.query(q, [date, batch_id], (err, data) => {
        if (err) return res.status(500).json(err);
        console.log(data);
        return res.status(200).json(data);
      });
